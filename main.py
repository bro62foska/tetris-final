...
class SoundManager:
  ...
  def _synth_move(self, path):
    ...
    for i in range(int(sr * dur)):
      t = i / sr
      val = 1.0 if math.sin(2 * math.pi * 260 * t) >= 0 else -1.0
      env = 1.0 - (i / (sr * dur))
      samples.append(val * 12000 * env)
  ...
  def _synth_rotate(self, path):
    ...
    for i in range(int(sr * dur)):
      t = i / sr
      freq = 450 + (i / (sr * dur)) * 150
      val = 1.0 if math.sin(2 * math.pi * freq * t) >= 0 else -1.0
      env = 1.0 - (i / (sr * dur))
      samples.append(val * 14000 * env)
    self._write_wav(path, samples, sr)
  ...
  def _synth_drop(self, path):
    ...
    for i in range(n_samples):
      t = i / sr
      freq = 300 - (i / n_samples) * 200
      val = 1.0 if math.sin(2 * math.pi * freq * t) >= 0 else -1.0
      env = 1.0 - (i / n_samples)
      samples.append(val * 18000 * env)
    self._write_wav(path, samples, sr)
  ...
  def _synth_punch(self, path):
    ...
    for i in range(n_samples):
      t = i / sr
      freq = 160 - (i / n_samples) * 100
      val = 1.0 if math.sin(2 * math.pi * freq * t) >= 0 else -1.0
      env = 1.0 - (i / n_samples)
      noise = (random.random() * 2.0 - 1.0) * 0.3
      samples.append((val * 0.7 + noise) * 22000 * env)
    self._write_wav(path, samples, sr)
...
class TetrisBoard(Widget):
  ...
  def draw_board(self, *args):
    ...
    self.block_size = min(
        self.width / GRID_WIDTH, available_height / GRID_HEIGHT
    )
